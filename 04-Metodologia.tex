    ...
    \section{Programa \textit{dtkintegrate}}
        ...
        \subsection{Clase VegasIntegrator}
            ...
            \begin{itemize}
                ...
                \item SetIntegrand: Recibe al campo especificado como integrando.
                \item SetIterations: Si el proceso iterativo no alcanza el mallado óptimo, se detendrá al alcanzar un número de iteraciones arbitrario para presentar el estimado acumulado.
                \item SetIntervals: Determina el tamaño fijo del mallado. Típicamente se establece en 1000.
                \item SetConvergenceRate: Se establece la taza de refinamiento de la malla. Este parámetro permite suavizar los cambios en el refinamiento del mallado.
                \item SetNumOfPoints: Determina el número de puntos de muestreo aleatorio Monte Carlo sobre la región de integración.
                \item SetTermalization: Indica el número de las primeras iteraciones cuya integral estimada no será utilizada para el estimado acumulado.
                \item SetTolerance: Dado que el mallado óptimo es casi imposible de alcanzar, se usa un rango de tolerancia.
                \item SetStopRefinement: Dado que la estimación acumulada puede sesgarse, se puede cortar su cálculo a partir de cierta j-ésima iteración para basarse únicamente en los resultados de integración Monte Carlo, los cuales muestran una distribución gaussiana con media en la integral verdadera.
                ...
                \item MaxDensity: Muestra el valor máximo de la densidad electrónica.
                ...
                \item SetNSamplesToFindMaximum: Para buscar el valor máximo de la densidad en el espacio de momentos, se realiza un muestreo aleatorio sobre la región de integración con un número de puntos arbitrario. Si se registra un valor de la densidad mayor a cero, se registra como máximo latente hasta que aparezca un valor al registrado. Por cada valor registrado, el radio de búsqueda se reduce de la región de integración a la distancia entre el origen y el último valor registrado. La oportunidad de realizar este método de rastreo se debe a que las densidades en el espacio de momento tienen comportamientos semejantes a una densidad gaussiana.
        ...
        \subsection{Eficiencia de \textit{dtkintegrate} contra el método Monte Carlo}
            Para corroborar que el programa \textit{dtkintegrate} presenta buena eficiencia, se desarrolló e implementó el algoritmo de integración Monte Carlo en \textit{DensToolKit} y se compararon ambos programas, tomando como referencia el porcentaje de error relativo contra el número de evaluaciones sobre el integrando. La función a integrar fue la densidad electrónica del metano, cuyo número de electrones es 10 y el resto de los parámetros del programa \textit{dtkintegrate} se mantuvieron fijos en los establecidos por defecto:
            \begin{itemize}
            ...
            \begin{figure}[htbp]
                ...
                \caption{Comparación del porcentaje de error relativo contra número de evaluaciones sobre la integral de la densidad electrónica del metano (10 electrones). Los métodos de integración usados fueron Monte Caro (azul) y Las Vegas+ (naranja), entre 10,000 y 1,000,000 de evaluaciones sobre el integrando.}
                \label{fig: mc_vs_lv}
            \end{figure}
    
    \section{Entropías de Shannon en átomos}
        \noindent Para corroborar el funcionamiento del programa implementado en \textit{DensToolKit}, se calcularon las entropías informacionales de las densidades normalizadas al número de electrones y a la unidad, así como de la densidad relativa, la cual toma como referencia su máximo global. La entropía se integró en átomos simples con números atómicos desde 1 hasta 36, utilizando como combinaciones método-base MP2/6-31+G* y HF/cc-pvTZ, ambos obtenidos por GAUSSIAN09 \cite{gaussian09}. Los resultados se compararon con aquellos publicados por Flores Gallegos, N. \cite{flores_gallegos} y Hô, M. et al. \cite{flores_gallegos,sagar}, quienes realizaron los mismos cálculos y con los mismos átomos, pero con combinaciones método-base CISD(full)/cc-pvTZ y HF/6-31G**, respectivamente. Los resultados mostrados en las figuras \ref{fig: entropies 1-36} resultaron ser muy semejantes a aquellos publicados por los autores mencionados anteriormente. Sin embargo, las pequeñas diferencias se deben al uso de diferentes bases o métodos.
        ...
        Los resultados mencionados anteriormente muestran que el programa \textit{dtkintegrate} se implementó adecuadamente en la suite \text{DensToolKit} y muestran la eficiencia esperada del algoritmo Las Vegas+ en funciones de comportamiento gaussiano, por lo que se procedió a realizar las comparaciones entre las distintas combinaciones de los métodos HF, MP2(full), MP3(full), MP4SDQ, B3LYP, wB97XD y las bases 6-31G, 6-31+G**, 6-31G(2df,p), cc-pVDZ, cc-pVTZ, aug-cc-pVDZ y aug-cc-pVTZ para átomos del Hidrógeno al Criptón.
        
    \section{Entropías de Shannon en moléculas simples}
        \noindent Se escogieron entre cinco moléculas y átomos que presentaran diez electrones en su estado neutral, pero que se diferenciaran entre ellos por su número de enlaces: Neón, ácido fluorhídrico, agua, amoniaco y metano. Todas ellas incrementan respectivamente su número de enlaces en una unidad desde cero hasta cuatro y las combinaciones método-base utilizadas fueron las mismas.
        
        Se calcularon las entropías informativas en la densidad normalizada al número de electrones, normalizada a la unidad y relativa, tomando como densidad de referencia su máximo global. Posteriormente, se analizó el comportamiento de las tres entropías conforme incrementaba el número de enlaces.