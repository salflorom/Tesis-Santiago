\chapter{Antecedentes}
    \section{DensToolKit}
        ...
        \begin{itemize}
            ...
            \item dtkdpdens, cuya tarea es evaluar uno de los campos implementados a lo largo del trayecto del enlace entre dos átomos,
        \end{itemize}
        mientras que un par de scripts suyos son
        \begin{itemize}
            \item dtkeps2pdf, cuya función es utilizar un archivo eps y transformarlo a formato pdf, y
            \item dtkpov2png, el cual se encarga de transformar archivos pov a png.
        \end{itemize}
        
        DensToolKit solicita como entrada archivos de formato \textit{wfn} ó \textit{wfx} que son producidos por programas tales como Gamess, NWChem, MOLPRO o Gaussian. Tales archivos contienen la expansión de la función de onda molecular o atómica en términos bases gaussianas. 
        
        No obstante la variedad de sus programas, no hay uno que se encargue de integrar los campos escalares mencionados, por lo que sería necesario implementar en él un algoritmo de integración para integrar cualquiera de los campos que ofrezca la suite.
    
    \section{Método Monte Carlo}
        \noindent Dada una función $f:\R^d\rightarrow\R$, con $d\in\N$, se puede encontrar su integral sobre un volumen $\Omega\in\R^d$ utilizando $M$ puntos aleatorios generados a partir de una densidad de probabilidad arbitraria $p(\bm{x})$, con $\bm{x}\in\R^d$. Numéricamente, es
        \begin{align}
            \Mean{f^l} = \frac{1}{M}\sum_{M}\Par{\frac{f(\bm{x})}{p(\bm{x})}}^l,
        \end{align}
        siendo $I$ la integral exacta de $f$ sobre el volumen $\Omega$, $l\in\N$ y de modo que $\Mean{f}\rightarrow I$ cuando $N\rightarrow M$.\\
        Su varianza se puede determinar a partir de la expresión
        \begin{align}
            \sigma^2 = \frac{\Mean{f^2}-\Mean{f}^2}{M-1}.
        \end{align}
        
        A pesar de que este método es igual de preciso independientemente de la función $f$ que se escoja, su desviación estándar se comporta de la forma
        \begin{align}
            \sigma = \sqrt{\frac{\Mean{f^2}-\Mean{f}^2}{N}},
        \end{align}
        forzando una tendencia más lenta hacia la integral $I$ cuantos más puntos de muestreo se realicen sobre $\Omega$. Es por eso que se han desarrollado métodos adaptativos que aceleran la tendencia mencionada anteriormente.
        ...
        \subsection{\textit{Muestreo estratificado}}
            \noindent Uno de los métodos para alcanzar la varianza $\sigma^2$ óptima es subdividiendo la región $\Omega$ en $N$ subconjuntos $\omega_i$, con $i\in\{1,2,\dots,N\}$, de tamaño variable, llamados incrementos. De ese modo, se puede integrar utilizando $M/N$ puntos de muestreo aleatorios en cada $\omega_i$.
                
            La varianza utilizando este método, $\sigma_s^2$, puede reducirse hasta un valor
            \begin{align}
                \sigma_s^2 \rightarrow \frac{\sigma^2}{N},
            \end{align}
            sin embargo el número de evaluaciones sobre $F$ crece al valor $M=2N^d$, volviéndose un serio problema cuando se trata de integrales en múltiples dimensiones.
        
    \section{Método Las Vegas+}
        \noindent A este método de integración también lo llamaremos Las Vegas-Monte Carlo y fue desarrollado por primera vez por \textit{Lepage, P.} \cite{lepage1978} en 1978. Este método fue ampliamente utilizado que incluso llegó a usarse en el CERN \cite{vegas-cern} y se implementó en diversos lenguajes de programación como en Fortran \cite{lepagefortran} o Python a través del programa VegasFlow \cite{vegasflow}. Además, se propusieron mejoras de la primera versión por parte de otros desarrolladores \cite{vegas-revisited} para el área de la física de partículas. La última mejora implementada sobre este método fue publicada por su propio autor en agosto del 2021 y lo llamó Las Vegas+ \cite{lepage2021}.
        
        La importancia de este método iterativo radica en su capacidad de adaptarse en dependencia del integrando para reducir la incertidumbre $\sigma^2$ prontamente, incrementando el número de evaluaciones a un orden $O(N_gd)$, en lugar de $O\Par{N_g^d}$.
        ...
        El procedimiento del método Las Vegas+ es generar un mallado uniforme y realizar un muestreo Monte Carlo para adquirir información del integrando, la cual será usada para modificar el mallado y realizar nuevamente un muestreo. Sucesivamente, el algoritmo recaudará cada vez más información para al final alcanzar un mallado óptimo y adaptado al comportamiento de la función a integrar.
        ...
        \end{align}
        Se procede al siguiente algoritmo.
        \begin{enumerate}
        ...
        Una vez generada la nueva malla, se reemplaza la anterior por esta, se realiza una muevo muestreo Monte Carlo y se repite el procedimiento hasta alcanzar el mallado óptimo y el estimado de la integral es suficientemente preciso.
        ...
        Dado que cada integración Monte Carlo arroja información sobre el integrando, se puede utilizar también para obtener un estimado acumulado y una varianza acumulada que ayudan a amortiguar la variación de ambas estimaciones, las cuales se pueden determinar por
        \begin{align}
        ...
        Por último, se debe verificar que los resultados obtenidos en cada iteración deben ser consistentes con respecto a las incertidumbres estimadas. La forma de hacerlo es a través de pruebas $\chi^2$, expresadas para este caso como
        \begin{align}
            \chi^2 = \sum_j\frac{\Par{I_j-\bar{I}}^2}{\sigma_j^2}.
        \end{align}
        
        Si $\chi^2$ llega a ser mayor al número de iteraciones presentes (menos uno), el proceso de integración debe reiniciarse.
    ...
    \section{Entropía de Shannon}
        \noindent Este concepto desarrollado por \textit{Shannon, C. E.} \cite{shannon2001} y nombrado como tal por su autor y semejanza con la entropía definida en la mecánica estadística se encarga de medir la cantidad de desinformación que presenta una variable aleatoria. Matemáticamente, se define como
        \begin{align}
        ...
        \begin{enumerate}
            ...
            \item Si todas las probabilidades son iguales, $p_i=\frac{1}{n}$, siendo $n$ el número de eventos posibles, entonces $H$ es una función monótona creciente y dependiente de $n$. Para probabilidades distintas, $H\leq \log{n}$. Esto quiere decir que los eventos con misma probabilidad de ocurrir, tienen más incertidumbre cuando hay un mayor número de posibles eventos.
            ...
            \item La entropía de un par de variables aleatorias es la entropía de una más la entropía condicional de la otra, es decir,
                \begin{align}
                    H(X,Y) = H(X)+H(X|Y).
                \end{align}
            ...
            \begin{align}\label{eq: desigualdad triangulo}
                H(X,Y) \leq H(X)+H(Y).
            \end{align}
            Esto quiere decir que la mínima incertidumbre que se puede alcanzar es cuando los eventos son independientes entre sí.
        ...
        \subsection{Aplicación de la entropía de Shannon en átomos y moléculas}
            \hl{Agrega el uso de la suma de las entrop\'ias en la qu\'imica computacional}.
            ...
            \begin{align}\label{eq: densidad elec}
                \rho(\bm{r}) \equiv N_{el}\int d\sigma_1d\bm{x}_2\cdots d\bm{x_N}\psi^*\Par{\bm{x}_1,\bm{x}_2,\dots,\bm{x}_N}\psi\Par{\bm{x}_1,\bm{x}_2,\dots,\bm{x}_N},
            \end{align}
            donde $\bm{x}_i=x(\bm{r}_i,\sigma_i)$, $\sigma_i$ es el espín del $i$-ésimo electrón y $N_{el}$ es el número de electrones que componen al átomo o la molécula, alberga la información del número de electrones por unidad de volumen en un punto del espacio de posiciones \cite{jensen}. Cabe notar que se considera un espacio 4-dimensional sobre $\psi$ al ser esta una función antisimétrica, por lo que el espin influye considerablemente en la localización de los electrones sobre el espacio 3-dimensional. Así mismo, la densidad electrónica puede estudiarse tanto en el espacio de posiciones, como en el espacio de momentos y su expresión es $\Pi=\Pi(\bm{p})$.
        
            A sabiendas de la utilidad de la función \eqref{eq: densidad elec}, la entropía de Shannon puede otorgar información sobre el grado de localización de los electrones en un átomo o molécula: Cuanto mayor sea su valor, mayor incertidumbre habrá sobre sus posiciones o momentos. Conocida la densidad electrónica de un átomo o molécula, las entropías de Shannon en el espacio de posiciones $S_{\rho}$ y en espacio de momentos $S_{\Pi}$ pueden expresarse como \cite{sagar}
            \begin{align}\label{eq: entropia N}
                S_{\rho}^{N_{el}}(\bm{r}) & = -\int\rho\!\Par{\bm{r}}\ln{\Sqbr{\rho\!\Par{\bm{r}}}}d\bm{r} \nonumber\\
                \text{y} & \\
                S_{\Pi}^{N_{el}}(\bm{p}) & = -\int\Pi\!\Par{\bm{p}}\ln{\Sqbr{\Pi\!\Par{\bm{p}}}}d\bm{p}. \nonumber
            \end{align}
            ...
            \begin{align}\label{eq: entropia rel 1}
                S_{\rho,r}^{N_{el}}\Par{\bm{r}} & = \int\rho(\bm{r})\ln{\Sqbr{\frac{\rho(\bm{r})}{\rho_0(\bm{r})}}}d\bm{r} \nonumber\\
                \text{y} & \\
                S_{\Pi,r}^{N_{el}}\Par{\bm{p}} & = \int\Pi(\bm{p})\ln{\Sqbr{\frac{\Pi(\bm{p})}{\Pi_0(\bm{p})}}}d\bm{p} \nonumber,
            \end{align}
            donde $\rho_0$ y $\Pi_0$ son densidades de referencia y por la definición de entropía relativa, $S_{\rho,r}^{N_{el}}$ y $S_{\Pi,r}^{N_{el}}$ miden la distancia entre ambas densidades correspondientes.
            ...
            \begin{align}\label{eq: entropias max}
                S_{\rho_{max}} & \equiv -\frac{\rho_{max}}{N}\int\Sqbr{\frac{\rho(\bm{r})}{\rho_{max}}}\log{\Sqbr{\frac{\rho(\bm{r})}{\rho_{max}}}}d\bm{r} \nonumber \\
                \text{y} & \\
                S_{\Pi_{max}} & \equiv -\frac{\Pi_{max}}{N}\int\Sqbr{\frac{\Pi(\bm{p})}{\Pi_{max}}}\log{\Sqbr{\frac{\Pi(\bm{p})}{\Pi_{max}}}}d\bm{p}. \nonumber
            \end{align}
        ...
        \subsection{Suma de las entropías de Shannon}
            ...
            \begin{align}\label{eq: suma entropias}
                S_{\rho}+S_{\Pi} \ge 3N_{el}\Par{1+\ln{\pi}}-2N_{el}\ln{N_{el}},
            \end{align}
            la cual muestra que habrá de presentarse una mínima incertidumbre entre las distribuciones de momentos y posiciones y depende del número de electrones del sistema. Sin embargo, si las densidades se encuentran normalizadas, aquél mínimo alcanzable es
            \begin{align}
                S_{\rho}^{1}+S_{\Pi}^{1} \ge 3\Par{1+\ln{\pi}}.
            \end{align}
            
            \textit{Gadre, S., Sears, S. et al.} \cite{gadre} propusieron que la suma de las entropías podría arrojar información sobre la eficiencia de un conjunto base que conforma a los orbitales atómicos y probó su hipótesis con los métodos y bases. \textit{Tripathi, A., Sagar, R. et al.} \cite{tripathi}, realizaron una prueba semejantes, pero con la serie isoelectrónica del Berilio. Por último, \textit{H{\^o}, M., Sagar, R. et al.} \cite{ho} mostraron estudios sobre la sensibilidad de las entropías $S_{\rho}$, $S_{\Pi}$ y $S_{\rho}+S_{\Pi}$ ante pequeñas variaciones de la densidad electrónica y sobre series isoelectrónicas de átomos.
                    
    \section{Método Hartree-Fock}
        \noindent También llamado método del campo autoconsistente, consiste en encontrar la función de onda de un átomo a través de varias iteraciones \cite{berthier,mcweeny,pople,roothaan}. En este método, se consideran funciones de onda antisimétricas y se expresan a través de determinantes de Slater.
        ...
        Por otro lado, el operador hamiltoniano asociado a un átomo de $2N$ electrones, utilizando unidades atómicas, es
        \begin{align}
            \hat{\Ham} & = -\frac{1}{2}\sum_{j=1}^{2N_{el}}\Lap{_j}{}-\sum_{j=1}^{2N_{el}}\frac{Z}{r_j}+\sum_{j=1}^{2N_{el}}\sum_{j>i}\frac{1}{r_{ij}} \nonumber\\
            & = \sum_{j=1}^{2N_{el}}\hat{h}_j+\sum_{j=1}^{2N_{el}}\sum_{j>i}\frac{1}{r_{ij}}, \label{eq: hamiltoniano}
        \end{align}
        donde
        \begin{align}
        ...
        La razón por la que se mencionan 2$N_{el}$ electrones es para definir $N_{el}$ electrones por cada espín.
        
        El método Hartree-Fock resuelve cada espín orbital a través del operador Fock. Al referirnos con $\bm{\tau}_1$ y $\bm{\tau}_2$ como las coordenadas espín-espaciales donde queremos ver la interacción entre dos electrones y al expresar $\varphi_i$ como el i-ésimo espín orbital de un electrón,
        \begin{align}\label{eq: fock equation}
            \hat{F}(\bm{\tau}_1)\varphi_i(\bm{\tau}_1) = \varepsilon_i\varphi_i(\bm{\tau}_1),\quad i=1,2\dots,N_{el},
        \end{align}
        cuyo valor propio asociado al i-ésimo espín orbital es la energía orbital de Hartree-Fock, $\varepsilon_i$.
        
        Si uno supone un sistema de capa cerrada, es decir que los orbitales se encuentran doblemente ocupados, el operador Fock puede depender únicamende de las coordenadas espaciales y expresarse como
        \begin{align}
            \hat{F}(\bm{r}_1) = \hat{h}(\bm{r}_1)+\sum_{j}^{N_{el}}\Sqbr{2\hat{J}_j(\bm{r}_1)-\hat{K}_j(\bm{r}_1)}, 
        \end{align}
        ...
        \end{align}
        donde las funciones de onda orbitales ahora son espaciales, $\psi$. Estas son \textit{ecuaciones restringidas de Hartree-Fock}, sin embargo, no es necesario que se aplique la condición de capa cerrada para encontrar las soluciones, pues también se puede derivar el operador de Fock para casos de capa abierta y sus ecuaciones se llamarán \textit{ecuaciones no restringidas de Hartree-Fock}.
        ...
        Sin importar si el sistema es una molécula o un átomo, uno puede suponer un orbital molecular que se compone de una combinación lineal de orbitales atómicos (MO-LCAO),
        ...
        Esta propuesta de función de onda implica en trabajar con un sistema de ecuaciones llamadas \textit{ecuaciones de Hartree-Fock-Roothaan} \cite{roothaan}, las cuales pueden manipularse como matrices herméticas de tamaño $k\times k$,
        \begin{align}
    ...
    \section{Métodos post-Hartree-Fock}
        ...
        \subsection{Métodos de interacción de configuraciones}
            ...
            Las expresiones $\Bra{\Psi}\Ham\Ket{\Psi}$ y $\Braket{\Psi}{\Psi}$ en términos de los coeficientes de la expanción de $\Psi$ pueden mostrarse como
            \begin{align}
            ...
            \end{align}
            depende únicamente de su propia variable, podemos resolver el problema por valores propios, es decir,
            \begin{align} \label{eq: matriz_1 ci}
                ...
                \end{pmatrix},
            \end{align}
            para proceder a la búsqueda de la energía mínima y los coeficientes $c_i$, con $i\in\{0,1,...\}$. La energía es el menor valor propio de \eqref{eq: matriz_1 ci} y los vectores propios son los coeficientes de los conjuntos de la función de onda \eqref{eq: psi ci}. El cálculo de los elementos $\Bra{\psi_i}\Ham\Ket{\psi_j}$, varía en dependencia de la pareja de orbiales $\psi_i$ y $\psi_j$, el cual puede verse a myor detalle por \textit{Jensen, F.} \cite{jensen}.
            
            Cuando se consideran todos los posibles determinantes de Slater por cada excitación, el método es CI(full), pero este método exige un coste computacional muy elevado, por lo que se suele cortar la combinación lineal de determinantes de Slater en las excitaciones o hasta las interacciones simples CIS, dobles CISD, triples CIST, etcétera. Por ejemplo, si se corta la serie hasta las excitaciones dobles, pero se consideran todos los determinantes de Slater por cada tipo de excitación, se tiene el método CISD(full).
            
        \subsection{Métodos perturbativos M\o ller-Plesset}
            \noindent Son métodos pos-Hartree-Fock que se caracterizan por usar la teoría perturbativa de Rayleigh-Schr\"odinger como estrategia para superar el límite Hartree-Fock y se pueden realizar perturbaciones de segundo orden (MP2) \cite{mp2-1}, tercero (MP3) \cite{mp3-1}, cuarto (MP4) \cite{mp4-1} e incluso más. MP2 puede tomar en cuenta entre el 80\% y 90\% de la energía de correlación, MP3 entre el 90\% y 95\%, y MP4 entre el 95\% y 98\% \cite{jensen}. Sobre los métodos subsecuentes, no se suele tener mucha información, puesto que requieren mucho costo computacional.
        
            En este método, el hamiltoniano no perturbado es la suma de los operadores de Fock,
            \begin{align}
                \hat{\Ham}^{(0)} & = \sum_{j=1}^{N_{el}}\hat{F} \nonumber\\
            & = \sum_{i=1}^{N_{el}}\hat{h}_i+\sum_{i=1}^{N_{el}}\sum_{j=1}^{N_{el}}\Mean{\frac{1}{r_{ij}}},
            \end{align}
            ...
            \begin{align}
                ...
                & = \Par{\sum_{i=1}^{N_{el}}\hat{h}_i+\sum_{i=1}^{N_{el}}\sum_{j>i}\frac{1}{r_{ij}}}-\Par{\sum_{i=1}^{N_{el}}\hat{h}_i+\sum_{j=1}^{N_{el}}\sum_{j=1}^{N_{el}}\Mean{\frac{1}{r_{ij}}}} \nonumber\\
                & = \sum_{i=1}^{N_{el}}\sum_{j>i}\frac{1}{r_{ij}}-\sum_{i=1}^{N_{el}}\sum_{j>i}
                \Mean{\frac{1}{r_{ij}}}.
            \end{align}
            Entonces, por la teoría perturbativa de Rayleigh-Schr\"odinger, la energía de orden cero de la energía es
            \begin{align}
    ...
    \section{Funcional B3LYP}
        ...
        Para resolver el problema, se requiere conocer $E_{XC}$, sin embargo, hasta ahora sólo se saben aproximaciones de la misma a través de diversos métodos, entre ellos aquellos que arrojan funcionales híbridos como B3LYP, donde suponen que la energía $E_{XC}$ formarse por una combinación lineal de las energías de intercambio $E_{X}$ y correlación $E_{C}$ individuales y que además, ambas se deducen por diversos métodos, entre ellos, Hartree-Fock. 
        
        La energía funcional de B3LYP es 
        \begin{align}
            E_{XC}^{B3LYP} = (1-a)E_X^{LDA}+aE_X^{HF}+bE_X^{B88}+(1-c)E_C^{LDA}+cE_C^{LYP},
        \end{align}
        donde $E_X^{LDA}$, $E_X^{HF}$ y $E_X^{B88}$ son las energías de intercambio deducidas a través de los métodos de aproximación de la densidad local ($LDA$), Hartree-Fock ($HF$) y por \textit{Becke, A.} ($B88$) \cite{becke}, respectivamente, mientras que $E_C^{LDA}$ y $E_C^{LYP}$ son sus respectivas energías de intercambio. Los coeficientes $a$ y $b$ se ajustan experimentalmente.
        
    \section{Conjuntos base de gaussianas}
        \noindent El conjunto base al que nos referiremos es a un combinación lineal de funciones que sirven para conformar a un orbital atómico. Existen varios tipos conjuntos base, pero las más utilizadas son las de tipo gaussiana (GTO), debido a la baja exigencia computacional que requieren, a diferencia de otros orbitales como los de Slater (STO) \cite{slater} que requieren infinitos términos para representar a un orbital molecular. Incluso, aunque no representan adecuadamente el comportamiento electrónico de un sistema cerca del núcleo, electrones core, ni muy lejos del mismo, electrones de valencia, combinaciones adecuadas de las mismas pueden arrojar un comportamiento aceptable de una función de onda Hartree-Fock \eqref{eq: funcion onda hf}.
        
        Una función gaussiana cartesiana primitiva es
        \begin{align}\label{eq: gaussiana}
            g_{ijk}(x,y,z) = Nx_b^iy_b^jz_b^ke^{-\alpha r_b^2},\quad N \equiv \Par{\frac{2\alpha}{\Pi}}\Sqbr{\frac{(8\alpha)^{i+j+k}i!j!k!}{(2i)!(2j)!(2k)!}}^{1/2},
        \end{align}
        donde $x_b$, $y_b$ y $z_b$ son las coordenadas del núcleo $b$-ésimo, $\alpha>0$ es un parámetro arbitrario y los valores $i$, $j$ y $k$ indican el tipo de gaussiana que se desee crear. Aunque también se pueden representar en coordenadas esféricas. La razón de llamarla primitiva es porque se pueden formar gaussianas a partir de combinaciones lineales de funciones primitivas.
        
        Las combinaciones lineales de estas funciones \eqref{eq: gaussiana} se adecúan para cada orbital molecular e incluso llegan a reforzar a los orbitales de capas interna y de valencia formando combinaciones lineales de gaussianas para formar gaussianas más adecuadas para el caso. La base más común es 6-31G \cite{gto}, la cual utiliza una combinación lineal de 6 gaussianas para los orbitales de capa interna, mientras los orbitales de valencia se conforman por un desdoblamiento de una combinación de 3 gaussianas y una gaussiana primitiva. 
        Para casos donde la nube electrónica se polariza, se agregan funciones de polarización a los orbitales de valencia, como es el caso de la base 6-31G* ó 6-31G(d), en el cual las funciones de polarización se agregan en los orbitales d para los átomos pesados. Otro ejemplo es 6-31G** ó 6-31G(d,f), donde se agregan además en los orbital f. Por último, si hay presencia electrónica en zonas muy alejadas del núcleo, se agregan funciones de difusión que extiendan a la función de onda aún más, como la base 6-31+G. Estas funciones se caracterizan por generalmente establecer al parámetro $\alpha$ entre los valores 0.01 y 0.1. Todas las bases GTO mencionadas anteriormente pueden mezclarse entre sí para representar de manera más adecuada al sistema, por ejemplo 6-31+G*.
        
        Otras bases gaussianas conocidas son las de tipo cc-pVXZ \cite{cc-pvxz}, las cuales quieren decir correlación consistente con funciones X Zeta de valencia polarizadas, donde X puede ser doble (D), triple (T), cuádruple (Q), etcétera. Además, a estas bases también se les pueden agregar gaussianas difusas, renombrándose aug-cc-pVXZ. Estas bases se caracterizan por su uso en métodos que incluyen correlación electrónica, como son los métodos post-Hartree-Fock. 